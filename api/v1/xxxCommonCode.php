...
$app->post('/GetUser', function() use ($app){
    ...
    if ($users != NULL) {
		...
		foreach ($users as $user) {
			...
			$userHotels = $db->getRecords("select ifnull(P.ID,0) ID ,ifnull(P.UserID,$id) UserID, ifnull(P.HotelID, C.GroupID) HotelID, C.Name HotelName , P.DefaultSelection, P.CurrentlyBrowsing from (select distinct Name, GroupId from channel where parentid=1) C
		...
		if($user['Password']==$password)
		{
			...
			/* if(isset($_SESSION['uid']))
			{
				$userHotels = $db->getRecords("select C.* from 
												(select distinct Name, GroupId from channel where parentid=1) C
						...
						if ($HotelUserDetails != NULL) {
							...
							foreach ($HotelUserDetails as $HotelUserDetail) {
								...
								$othersql="Select 
                                 ...
                                 JOIN channel c on (c.GroupId = UH.HotelID)
...
$app->get('/GetHotelMenu', function()  {
	...
	{
		$sql = "select *, '+' CollapseLabel, 0 isChecked from channel ";
	}
	else
	{
		$sql = "select *, '+' CollapseLabel, 0 isChecked from channel where parentid=0
						union
...
$app->get('/LoadDashboardConfigs', function()   
{
	...
	{ // Load channel
		if($session['isadmin']=="1")
		{
			$sql = " select *, '+' CollapseLabel, 0 isChecked, (case when I.red is not null and I.red=1 then 'red' else 'white' end) color , 0 CurrentlyBrowsing, 0 DefaultSelection from channel c
		...
		{
			$sql = " select *, '+' CollapseLabel, 0 isChecked, 'white' color, 0 CurrentlyBrowsing, 0 DefaultSelection  from channel where parentid=0
							union
		...
		if ($hotels != NULL) {
			...
			if($session['isadmin']==1)
			{
				$GetAllWidgets = $db->getRecords("SELECT * FROM power.Portal_WidgetsMaster");
			}
			else
			{
				...
				$GetAllWidgets = $db->getRecords("SELECT W.* FROM power.Portal_WidgetsMaster W 
	...
	{ // Load weather
		...
		if($session['isadmin']=="1")
		{
			$sql = "		select date_format(date, '%Y-%m-%d') date,date_format(date, '%d-%m') date1, temp, hdd, cdd, n.Name
							from weather c
							join (select groupid, Name from channel where parentid=1 and GroupId = ".$r->dparams->SelectedHotel.") n
...
$app->post('/UpdateOccupancy', function() use ($app) {
	...
	else
	{
		...
		if ($occExists == NULL) {
			...
			$hotelName = "";
			$getHotelName = $db->getOneRecord("select Name from channel where GroupId = ".$r->Occupancy->hotelid);
...
(case when exists(select 1 from portal_InvalidFunctionalGroups where FGName='F&B' and HotelID=c.hotelid) then 0 else 1 end) fb_IsValid,
			   ...
			   from smdata c
			   join (select Name, GroupId from channel where parentid=1 and GroupId = ".$r->dparams->SelectedHotel.") p
		...
		/*$AllHotels = $db->getRecords("select * from energy.channel where parentid=1");
...
$app->post('/LoadExcelPrintDetail', function() use ($app)  
{
	...
	{ 
		...
		$NewDayQuery = "select *,date_format(date,'%a') dname,date_format(date,'%d/%m/%Y') dt1,date_format(fdtime,'%H:%i') fdt,date_format(ldtime,'%H:%i') ldt,
				(select Name from channel where GroupId=".$r->dparams->SelectedHotel.") hotelName
		...
		count( distinct (case when r.post_parent=0 then r.ID else r.post_parent end)) alertcount from channel c
					left join (

					select *,getTopParentPositionId(groupid) grpid from energy.valert ) r
		...
		/*$alertCountSql ="		select c.fid groupid, c.fname Name, count(*) alertcount from 
					(
						select b.name, b.groupid,c.groupid fid, c.name fname from channelext c
						join  channelext b
		...
		$sql="select O.*, T.Code, c.Name wherename from Portal_OBS O JOIN Portal_ObservationsType T on (O.ObservationTypeID=T.ID) join channel c on (c.GroupId=O.where) where HotelID=".$r->dparams->SelectedHotel." and ifnull(Deleted,0)=0 and PriorityNumber is not null and PriorityNumber between 1 and 10 order by PriorityNumber 
					";
...
$app->post('/LoadExcelPrintDetailsOnDateChange', function() use ($app)  
{
	...
	$smprintQuery="Select * from smdata_PrintDetails where Hotelid='".$r->dparams->SelectedHotel."'and date_format(CreatedDate,'%Y-%m-%d')='".$r->dparams->date."' order by ID desc";
		...
		/*$alertCountSql ="select c.groupid, c.Name, count(*) alertcount from channel c
					left join (
...
$app->post('/UpdateExcelPrintDetails', function() use ($app) {
	...
	else
	{
			...
			$sql = "INSERT INTO power.smdata_PrintDetails
					(
					userid,
					Hotelid,
					fbFN,
...
$app->post('/LoadAlert', function() use ($app)  
{
	...
	{ // Load weather
			...
			$AlertSetsql="	select
						...
						A.sendto,
						concat(A.sendfrom,',',
						ifnull((select N.meta_value from energy.nlk_users U
							JOIN energy.nlk_usermeta N
							ON (U.ID=N.user_id)
					...
					from
						...
						ON (AlertDayTab.Code = CAST(A.AlertDay as char))
						 JOIN
						(
							select distinct groupid,name from energy.channelext1
							where path like concat('%/',(select distinct groupid from channel where groupid =".$r->dparams->SelectedHotel."),'/%')
						) as chn
		...
		if ($AlertSetDetails != NULL) {
			...
			SELECT V.ID, V.name,email,
				...
				,datediff(closed,date) as daysopen,
	                        (
	                            select name from channel where groupid in (select getTopParentPositionId(V.groupid) )
	                                ) FGName
						FROM energy.valert V
...
$othersql="Select 
			...
			$sql = "SELECT hotelid, min(date) as date, nl, dm,
					...
					end as season  FROM (SELECT hotelid ,nl,dm, date FROM power.smdata 
...
$app->post('/LoadCircuitDetails', function() use ($app)    
{
	...
	{ // Load channel
			...
			from
			(
				select distinct c.Name, c.GroupId , c.Type, cx.GroupId grpid from channel c 
				left join channelext cx on (cx.path like concat('%/',c.GroupId,'/%'))
				where c.Parentid=".$r->dparams->SelectedHotel."
			) k
			left join
			Portal_Circuit Cir
	...
	{ // Load channel
		...
		if ($ObservationsTypes != NULL) {
			...
			foreach ($ObservationsTypes as $ObservationsType) {
				...
				$sql = "select -1 GroupId, 'All' Name union select distinct GroupId, Name  from channelext where path like '%/".$ObservationsType['where']."/%'";
		...
		$sql = "select distinct GroupId, name Name  from channel where parentid= ".$r->dparams->SelectedHotel;
		$funcgr = $db->getRecords($sql);
		if ($funcgr != NULL) {
			$funitems = array();
			foreach ($funcgr as $fun) {
			...
			$sql = "select -1 GroupId, 'All' Name union select distinct GroupId, Name  from channelext where path like '%/".$ObservationRes['FuntionalGroups'][0]['GroupId']."/%'";
			$funcgr = $db->getRecords($sql);
			if ($funcgr != NULL) {
				$funitems = array();
				foreach ($funcgr as $fun) {
...
$app->post('/LoadLogActivityDetails', function() use ($app)    
{
	...
	{ // Load channel
			...
			LEFT JOIN (select groupid , Name from channel where parentid=1) H ON (L.HotelID= H.groupid)
			where 
...
$app->post('/LoadCircuitsForFG', function() use ($app)    
{
	...
	{ // Load channel
			...
			$sql = "select -1 GroupId, 'All' Name union select distinct GroupId, Name  from channelext where path like '%/".$r->dparams->FunctionalGroupID."/%'";
...
$app->post('/EmailHtml', function() use ($app)    
{
	...
	{ // Load channel
		...
		$mail->Host = 'mail.responsiblepower.co.uk';  // Specify main and backup SMTP servers
		$mail->SMTPAuth = true;                               // Enable SMTP authentication
		$mail->Username = 'portal@responsiblepower.co.uk';                 // SMTP username
		$mail->Password = 'mortal-portal';                           // SMTP password
		$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
		$mail->Port = 587;                                    // TCP port to connect to

		$mail->setFrom('portal@responsiblepower.co.uk', 'Responsible Power');
...
$app->post('/GetAllContactFormMessages', function() use ($app)    
{
	...
	{ // Load channel
			...
			select m.*, u.Firstname, u.Lastname, c.Name HotelName,
                ...
                left join channel c on (m.HotelID=c.GroupId)
                where 
...
$app->post('/GetAllNewContactFormMessages', function() use ($app)    
{
	...
	{ // Load channel
			...
			$sql2 = "
				...
				from Portal_ContactFormMessages m 
                join Portal_Users u on (u.ID = m.UserID)
                join channel c on (m.HotelID=c.GroupId)
